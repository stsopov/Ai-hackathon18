...
# sort data
dall <- dall[order(dall$id, dall$date, dall$time),]

# find data of the first purchase
dall$first_prch <- paste0("20", substr(dall$first_prch, 7, 8), "-",
                          substr(dall$first_prch, 4, 5), "-",
                          substr(dall$first_prch, 1, 2), " ",
                          substr(dall$first_prch, 10, 17)
)



dall$first_p_month <- paste0(substr(dall$first_prch, 1, 8), "01")

# lag features
dall[, previous_date := shift(date, 1L, type = "lag"), by = "id"]
dall[, previous_v_l := shift(v_l, 1L, type = "lag"), by = "id"]
dall[, previous_sum_b := shift(sum_b, 1L, type = "lag"), by = "id"]
dall[, previous_percent := shift(percent, 1L, type = "lag"), by = "id"]
dall[, previous_q := shift(q, 1L, type = "lag"), by = "id"]


dall$date_diff <- -as.numeric(difftime(as.Date(dall$previous_date), as.Date(dall$date), units = "days"))

all.month.window.stat.list <- list()
...
for (m in c("2017-09-01", '2017-11-01')) {
  ...
  stat_all_time <- transaction.pars %>% group_by_(.dots = "id") %>% summarize(
    ...
    last_purchase_date = date[1],
    last_percent = percent[1],
    last_location = location[1],
    last_type = type[1],
    last_region = region[1],
    last_azs = code_azs[1],
    last_q = q[1],
    last_sum_b = sum_b[1],
    last_v_l = v_l[1],
  ...
  stat_all_time$month <- m
  stat_all_time$days_passed_after_first_p <- as.numeric(difftime(stat_all_time$month, stat_all_time$f_month, units = "days"))
  stat_all_time$day_last_purchase <- day(stat_all_time$last_purchase_date)
  
  all.month.window.stat.list[[length(all.month.window.stat.list) + 1]] <- stat_all_time
}

all.stat <- rbindlist(all.month.window.stat.list)
all.stat[is.na(all.stat)] <- 0

save(all.stat, file = "Data/prepared/all_stat")
