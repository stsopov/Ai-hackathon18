this.last.day.window.stat.list <- list()
...
for (m in c("2017-09-01", '2017-11-01')) {
  ...
  transaction.pars[,max_date := max(date), by = "id"]
  transaction.pars <- transaction.pars[transaction.pars$date == transaction.pars$max_date,]
  ...
  stat_this_last_day <- transaction.pars %>% group_by_(.dots = "id") %>% summarize(
  ...
  stat_this_last_day$month <- m
  colnames(stat_this_last_day)[!colnames(stat_this_last_day) %in% c("month", "id")] <- 
    paste0("this_last_day_", colnames(stat_this_last_day)[!colnames(stat_this_last_day) %in% c("month", "id")])
  
  this.last.day.window.stat.list[[length(this.last.day.window.stat.list) + 1]] <- stat_this_last_day
}

this.last.day.stat <- rbindlist(this.last.day.window.stat.list)
this.last.day.stat[is.na(this.last.day.stat)] <- 0

save(this.last.day.stat, file = "Data/prepared/this_last_day_stat")
